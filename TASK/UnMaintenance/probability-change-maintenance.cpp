        ...
        for (int j = 0; j < d; j++) {
            ...
            if (i < u) {    //确保每条边只处理一次
                if (rand() > scale_size) {  //待增加
                    ...
                    vector<vector<double> > range = increase_threshold_compute_candidate(thres, u, i, tm_1, candi_count);  //顶点为u、i
                    candidate_tm += tm_1;
...
//边缘概率增大 -- 寻找候选集 并更新
vector<vector<double> > Probability_Core::increase_threshold_compute_candidate(vector<vector<double> >& thres, int u, int v, double& time, int& ct) {
        ...
        for (int j = 0; j < d; j++) {
            ...
            if (i < u) {    //确保每条边只处理一次
                if (rand() > scale_size) {  //待增加
                    ...
                    int mincore = min(core[u], core[i]);
                    vector<vector<double> > range = decrease_threshold_compute_candidate(thres, u, i, tm_1, candi_count);  //顶点为u、i
                    candidate_tm += tm_1;
                    candi_sum += candi_count;
...
//边缘概率减小 -- 寻找候选集 并更新
vector<vector<double> > Probability_Core::decrease_threshold_compute_candidate(vector<vector<double> >& thres, int u, int v, double& time, int& ct) {
