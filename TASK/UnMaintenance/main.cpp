...
vector<vector<double> > insertEdges(string infile, string parentPath, double scale) {
	...
	//uc.edge_selected_creat_bin(infile, 0.8, 340);	//读取bin文件，加入340条边，并存储初始图
...
int main()
{
	...
	const char* coreFile = "F:/申博/ZJU/uncertain graphs/uncertain graph/datas/core_num_core.txt";
	...
	//pc.increase_threshold_compare(thres, 0.8, 100);	//边缘概率增加
	//pc.decrease_threshold_compare(thres, 0.8, 100);	//边缘概率减小
